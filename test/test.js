...
test('It should only update once if the same game is added to the queue multiple times', (t) => {
  const masters = [];
  const watcher = new Watcher({
    scores,
    year,
    ...
    onDrain() {
      t.equal(masters.length, 1);
      t.equal(masters[0], constants.EMPTY.replace('MWX', 'MW1'));
      t.end();
    }
  }).scores;
